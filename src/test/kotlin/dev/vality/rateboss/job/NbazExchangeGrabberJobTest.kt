...
@SpringBootTest(
    properties = [
        "rates.nbaz-job.jobCron=0/5 * * * * ?",
        "rates.nbaz-job.currencies.[0].symbolCode=AZN",
        "rates.nbaz-job.currencies.[0].exponent=2",
    ],
)
class NbazExchangeGrabberJobTest : ContainerConfiguration() {
    @MockitoSpyBean
    lateinit var exchangeDaoService: ExchangeDaoService

    @MockitoBean
    lateinit var nbazExchangeRateSource: NbazExchangeRateSource
    ...
    fun `test grabber job`() {
        whenever(nbazExchangeRateSource.getSourceId()).thenReturn("sourceId")
        whenever(nbazExchangeRateSource.getExchangeRate(any())).then {
            ExchangeRates(
                rates =
                    mapOf(
                        "USD" to BigDecimal.valueOf(1.7),
                        "EUR" to BigDecimal.valueOf(1.977),
                    ),
                timestamp = Instant.now().epochSecond,
            )
        }
        scheduler.triggerJob(JobKey(ratesProperties.nbazJob.jobKey))
